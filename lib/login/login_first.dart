...
class _LoginFirstState extends State<LoginFirst> {
  ...
  void _wechatLoginCallback() {
    ...
    if (res is WeChatAuthResponse) {
      ...
      if (authResponse.state == 'sufenbao_login' && authResponse.errCode == 0 && authResponse.code != null) {
  ...
  _createButton(bool wechat) {
    if (wechat) {
      return CustomButton(
        ...
        leftTextSize: 5,
        text: '微信登录',
        style: CustomButtonStyle.wechat,
        onPressed: () {
          if (checkboxSelected) {
            getCode();
          } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        padding: EdgeInsets.fromLTRB(20, 20 + 60, 20, 20),
        child: Column(
          children: [
            ClipRRect(
            ...
            Center(
              child: Column(
                children: [
                  showWechatLogin ? _createButton(true) : SizedBox(),
