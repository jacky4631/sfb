...
class _PhonePage extends State<PhonePage> {
    ...
    if (text.length > inputLength) {
      ...
      if (text.length == 4 || text.length == 9) {
        ...
        _mobileController.selection =
            TextSelection.fromPosition(TextPosition(affinity: TextAffinity.downstream, offset: text.length)); //光标移到最后
      }
    } else {
      //删除
      if (text.length == 4 || text.length == 9) {
  ...
  Future modifyPhone() async {
    ...
    if (res['success']) {
      Navigator.pushNamed(context, "/phoneCodePage", arguments: {'mobile': _mobileController.text.toString()});
    } else {
